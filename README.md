...
`config.host.yaml` 的 AnyTLS（TCP `14444`）和 VLESS/TLS（TCP `14443`）共用仓库内仅供模板和测试的自签名 `server.crt` / `server.key`，其 SAN 为 `speedtest.net`；该证书不受该域名信任，部署前必须替换为自行持有且由客户端信任的证书和私钥，并替换 VLESS UUID。
